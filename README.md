...
Three runnable training scripts (`--help` on each for the full config):
...
See [`docs/api.md`](docs/api.md) for the full reference (arguments, connectomes,
decoders, global toggles).
...
```
silogic/            the installable package (see docs/api.md)
...
- **`docs.yml`** — builds the MkDocs Material site (`mkdocs.yml` + `docs/`) and
