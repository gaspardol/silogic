...
the library rather than the flat FC ``LogicNet``:
  ...
  * ``get_cifar_spatial`` — CIFAR-10 as binary spatial channels: a 3-level
    thermometer code per colour PLUS Sobel/Laplacian edge channels (the single
    biggest accuracy lever), with crop+flip augmentation.
  * ``LogicTreeNet`` — Petersen-style logic-gate-tree convolutions
    (``ConvLogicTree``) + logical OR pooling (``OrPool``) + residual init, with
    learnable Top-K leaf connectivity, finished by a dense logic head + GroupSum.
  * ``train_model`` — AdamW + cosine LR with soft + hard evaluation.

This default config reaches **~60% soft / ~57% hard** test accuracy in **~10–12
minutes** on a GPU (channels [256, 512], ~2.8M gates). It is fast because the
learnable Top-K conv has a fused Triton kernel (`tree_conv_topk`); scale the
channels up for higher accuracy at proportional cost.

    python examples/train_cifar10.py
    python examples/train_cifar10.py --channels 384 768 --epochs 40   # higher acc
"""
import argparse
import torch
...
def main():
    ...
    p.add_argument("--channels", type=int, nargs="+", default=[256, 512],
    ...
    print(f"Loading CIFAR-10 spatial (3-level thermometer + "
    ...
    # compile_ off: the conv path mixes unfold + custom Triton kernels that don't
    # always trace cleanly; the fused Top-K conv kernel does the heavy lifting.
